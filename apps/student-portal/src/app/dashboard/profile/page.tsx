...
export default function ProfilePage() {
      ...
      if (data.skills) {
        setSkills(
          data.skills.map((s: any) => ({
            id: s.id,
            skillId: s.skillId,
            // Server returns skillName (flat), not a nested skill object
  ...
  const handleAddSkill = async () => {
    ...
    try {
      ...
      // Re-fetch profile so the new userSkill gets its DB id (needed for deletion)
      await fetchProfile();
      setShowAddSkill(false);
      setNewSkillId("");
      setNewSkillLevel("beginner");
      const added = availableSkills.find((s) => s.id === newSkillId);
      toast.success(`${added?.name ?? "Skill"} added!`);
    } catch (err: any) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Re-fetch from server to ensure local state matches persisted DB row
      await fetchProfile();
              ...
              <div className="flex-1">
                ...
                >
                  ...
                  {availableSkills
                    .filter(
                      (s) =>
                        !skills.some((existing) => existing.skillId === s.id),
                    )
                    .map((s) => (
