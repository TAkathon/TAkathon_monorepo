...
const MILESTONES = [
  ...
  {
    id: 2,
    title: "Architecture & Tech Stack Decision",
    status: "done",
    due: "Day 1",
  },
  ...
  {
    id: 6,
    title: "Demo Preparation & Submission",
    status: "pending",
    due: "Day 4",
  },
];
...
function MilestoneStatusBadge({ status }: { status: string }) {
  if (status === "done")
    return (
      <span className="px-2 py-0.5 bg-green-500/15 text-green-400 border border-green-500/30 rounded-full text-xs font-semibold">
        Done
      </span>
    );
  if (status === "in_progress")
    return (
      <span className="px-2 py-0.5 bg-primary/15 text-primary border border-primary/30 rounded-full text-xs font-semibold">
        In Progress
      </span>
    );
  return (
    <span className="px-2 py-0.5 bg-white/5 text-white/40 border border-white/10 rounded-full text-xs">
      Pending
    </span>
  );
}
        ...
        <div className="mb-2">
          <Breadcrumbs
            items={[
              ...
              { label: team?.name ?? "Team", href: `/dashboard/teams` },
              { label: "Project" },
            ]}
            showBack
          />
            ...
            <div className="glass rounded-xl p-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="text-white/75 text-sm mt-1 leading-relaxed">
                    Teams often struggle to coordinate effectively during
                    time-constrained hackathons. Our tool bridges communication
                    gaps and helps teams focus on building rather than managing.
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-3">
                {MILESTONES.map((milestone) => (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <p
                        className={`text-sm font-medium ${milestone.status === "done" ? "text-white/60 line-through" : "text-white"}`}
                      >
                        {milestone.title}
                      </p>
                    </div>
