...
import studentSettingsRouter from "./routes/students/settings";
...
import organizerSettingsRouter from "./routes/organizers/settings";
...
import notificationsRouter from "./routes/shared/notifications";
...
if (missingVars.length > 0) {
  if (IS_PRODUCTION) {
    ...
    console.warn(
      `[startup] WARNING: Missing env vars (using insecure dev defaults): ${missingVars.join(", ")}`,
    );
  }
}
// ─────────────────────────────────────────────────────────────────────────────
...
const allowedOrigins =
  CORS_ORIGINS.length > 0 ? CORS_ORIGINS : IS_PRODUCTION ? [] : DEV_ORIGINS;

if (!IS_PRODUCTION && CORS_ORIGINS.length === 0) {
  console.warn(
    "[CORS] CORS_ORIGINS not set — allowing default localhost origins for development:",
    DEV_ORIGINS.join(", "),
  );
}
  ...
  cors({
    origin: (
      ...
      if (allowedOrigins.length === 0) {
        return callback(
          new Error("CORS not configured — set CORS_ORIGINS env var"),
          false,
        );
      }
...
// Student settings: /api/v1/students/settings/change-password, /delete-account
app.use("/api/v1/students/settings", studentSettingsRouter);
...
// Organizer settings: /api/v1/organizers/settings/change-password, /delete-account
app.use("/api/v1/organizers/settings", organizerSettingsRouter);
