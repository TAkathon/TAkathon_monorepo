...
export default function OverviewPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (profileRes?.data?.data) {
        setProfileName(
          profileRes.data.data.fullName ||
            profileRes.data.data.user?.fullName ||
            "Organizer",
        );
      }
    } catch (error) {
  ...
  const stats = [
    ...
    },
    {
      ...
      change: `${hackathons.length} total`,
      changeType: "neutral",
      icon: Calendar,
    },
    {
            ...
            <div className="space-y-6">
                    ...
                    >
                      <div className="flex items-start justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <h4 className="text-white font-medium text-sm group-hover:text-primary transition-colors line-clamp-1">
                              {h.title}
                            </h4>
              ...
              <div className="bg-primary/10 border border-primary/20 rounded-xl p-5 space-y-2">
                ...
                <p className="text-xs text-white/70 leading-relaxed">
                  Keep your hackathon descriptions updated and publish them
                  early to attract more participants!
                </p>
              </div>
            </div>
          </div>
